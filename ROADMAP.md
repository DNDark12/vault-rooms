...
Research direction approved; execution is gated. Two review passes on 2026-07-20 conditionally approved Phase 0 and required Phase 1 to lock a fuller contract set before any schema/protocol/code. The plan runs Phase 0 hard gates (four-tier baseline-aware bundle scan, editor binding on two vaults, persistent-state + offline reconciliation) then Phase 1 written contracts 1.1–1.13 (eligibility, capability negotiation, bidirectional handshake, legacy-write policy, document epoch + **authoritative epoch source**, **first-create flow**, **ACL matrix incl. inbound sync_step2**, **room-mode/epoch delivery + live toggle**, **ACL-revoke ≠ destructive cleanup**, **one persistence strategy + startup ordering**, materialization-vs-compaction, resource limits, **persistence-failure invariant**). Do not start Phase 2+ until Phase 0 is green and Phase 1 contracts are reviewed — the gates are real go/no-go points, not formalities.
