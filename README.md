...
- **Embedded (in Obsidian):** configure the relay in Settings → Vault Rooms → Relay server (Public URL override, Port, Max synced file size, Start automatically), then use the Start/Stop button. Public URL override is manually maintained: after an address change, restart the server and use the panel's **Test LAN URL** action. A fresh server creates and persists its own pinned identity, listens on HTTPS/WSS, and bootstraps the owner through that pinned connection. There is no Host setting - the embedded server always binds every LAN interface (`0.0.0.0`). Nothing is read from `.env` in this mode.
...
- A teammate can't reach the server at all: on the host, confirm **LAN share: reachable from this device** and that the advertised URL uses the current LAN IP, not `127.0.0.1`. If it is unreachable, update Public URL override, restart the server, and click **Test LAN URL**. Then use **Test connection** on the teammate's plugin; a green host badge cannot detect the teammate's firewall or Wi-Fi client isolation, and a browser cannot validate the embedded server's private CA and internal TLS name.
