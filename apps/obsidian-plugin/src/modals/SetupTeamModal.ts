...
export class SetupTeamModal extends Modal {
  ...
  onOpen(): void {
    ...
    new Setting(contentEl).setName("Team name").addText((text) => text.setValue(this.teamName).onChange((value) => (this.teamName = value.trim())));
    ...
    new Setting(contentEl).addButton((button) =>
      button.setCta().setButtonText("Set up team").onClick(async () => {
        try {
          await this.plugin.setupTeam(this.serverUrl, this.teamName, this.displayName, this.deviceName);
          this.close();
        } catch (error) {
          new Notice(error instanceof Error ? error.message : "Team setup failed");
        }
      })
    );
  }
